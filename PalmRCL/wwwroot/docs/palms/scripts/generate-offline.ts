...
type PalmSpec = {
    palm: { id?: GUID; name: string; slug?: string };
    models: { id?: GUID; name: string; workflowStates?: number[] }[];
    lifecycles: { id?: GUID; name: string; modelIds: GUID[] }[];
    lifecyclePhases: { id?: GUID; name: string; lifecycleId: GUID; order: number }[];
    workflows: { id?: GUID; name: string; phaseId: GUID }[];
    workflowSteps: { id?: GUID; name: string; workflowId: GUID; order: number }[];
    phaseGates: { id?: GUID; name: string }[];
    lifecyclePhaseGates: { phaseId: GUID; gateId: GUID }[];
    workflowStepGates: { stepId: GUID; gateId: GUID; placement?: string }[];
    gateTransitions: {
        id?: GUID; gateId: GUID; name: string;
        mode: "Absolute" | "Relative";
        relativeKind?: "Next" | "Previous";
        priority?: number; isDefault?: boolean; conditionExpr?: string | null;
        targetPhaseId?: GUID | null; targetStepId?: GUID | null;
    }[];
};
...
function fm(meta: Record<string, any>) {
...
function detUuid(namespace: GUID, ...parts: string[]) { return uuidv5(parts.join("::"), namespace); }

function loadConfig(): Config {
    const cfgPath = path.join(ROOT, "palms.config.json");
    const cfg = readJSON<Config>(cfgPath);
    return cfg;
}

function loadStates(): WorkflowState[] {
    const p = path.join(DATA_DIR, "workflow-states.json");
    return readJSON<WorkflowState[]>(p);
}
...
function validate(spec: PalmSpec, states: WorkflowState[]) {
    const ids = <T extends { id?: GUID }>(arr: T[]) => new Set(arr.map(x => x.id!).filter(Boolean));
    ...
    {
        const key = (lifecycleId: GUID) => spec.lifecyclePhases.filter(p => p.lifecycleId === lifecycleId);
        for (const l of spec.lifecycles) {
    ...
    {
        const stepsByWf = new Map<GUID, { order: number; id: GUID; name: string }[]>();
        for (const s of spec.workflowSteps) {
            must(wfIds.has(s.workflowId), `Step '${s.name}' references missing Workflow ID=${s.workflowId}`);
            if (!stepsByWf.has(s.workflowId)) stepsByWf.set(s.workflowId, []);
    ...
    {
        const byGate = new Map<GUID, PalmSpec["gateTransitions"]>();
        for (const t of spec.gateTransitions) {
            if (!byGate.has(t.gateId)) byGate.set(t.gateId, []);
        ...
        for (const g of spec.phaseGates) {
            ...
            for (const t of arr) {
                if (t.mode === "Relative") {
                    if (!t.relativeKind) throw new Error(`GateTransition '${t.name}': Relative requires relativeKind`);
                    if (t.targetPhaseId || t.targetStepId) throw new Error(`GateTransition '${t.name}': Relative must not set absolute targets`);
                } else {
                    ...
                    if (t.targetPhaseId) must(phaseIds.has(t.targetPhaseId), `GateTransition '${t.name}' targets missing Phase`);
                    if (t.targetStepId) must(stepIds.has(t.targetStepId), `GateTransition '${t.name}' targets missing Step`);
                }
            }
        }
    }
...
function applyDeterministicIds(cfg: Config, spec: PalmSpec) {
    const ns = cfg.namespaceUUID;
    const assign = (id: GUID | undefined, type: string, name: string, parent?: string) =>
        id ?? detUuid(ns, type, parent ?? "", name);

    spec.palm.id = assign(spec.palm.id, "PaLM", spec.palm.name);

    for (const m of spec.models) m.id = assign(m.id, "Model", m.name, spec.palm.id);
    for (const l of spec.lifecycles) l.id = assign(l.id, "Lifecycle", l.name, JSON.stringify(l.modelIds.sort()));
    for (const p of spec.lifecyclePhases) p.id = assign(p.id, "LifecyclePhase", p.name, p.lifecycleId);
    for (const w of spec.workflows) w.id = assign(w.id, "Workflow", w.name, w.phaseId);
    for (const s of spec.workflowSteps) s.id = assign(s.id, "WorkflowStep", s.name, s.workflowId);
    for (const g of spec.phaseGates) g.id = assign(g.id, "PhaseGate", g.name);
    for (const t of spec.gateTransitions) t.id = assign(t.id, "GateTransition", t.name, t.gateId);
}

function page(meta: any, relations: string[], shorthand: string, children: { label: string; href: string }[], storage: StorageCfg) {
    return `${fm({
        type: meta.type,
        id: meta.id,
        name: meta.name,
...
function indexPage(cfg: Config, palm: { id: GUID; name: string; slug: string }, models: any[], lifecycles: any[], storage: StorageCfg) {
...
async function run() {
    ...
    for (const specPath of specs) {
        const spec = readJSON<PalmSpec>(specPath);
        ...
        for (const m of spec.models) {
            ...
            writeFile(path.join(base, "models", `${mSlug}-${m.id}.md`),
                page({ type: "Model", id: m.id, name: m.name, slug: mSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const l of spec.lifecycles) {
            ...
            writeFile(path.join(base, "lifecycles", `${lSlug}-${l.id}.md`),
                page({ type: "Lifecycle", id: l.id, name: l.name, slug: lSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const ph of spec.lifecyclePhases) {
            ...
            const children: { label: string; href: string }[] = [];
            if (wf) children.push({ label: `Workflow: ${wf.name}`, href: `/docs/palms/${palmSlug}-${palmId}/workflows/${slug(wf.name)}-${wf.id}.md` });
            for (const g of gatesAtPhase) children.push({ label: `Gate: ${g.name}`, href: `/docs/palms/${palmSlug}-${palmId}/gates/${slug(g.name)}-${g.id}.md` });

            writeFile(path.join(base, "phases", `${String(ph.order).padStart(2, "0")}-${phSlug}-${ph.id}.md`),
                page({ type: "LifecyclePhase", id: ph.id, name: ph.name, slug: phSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const w of spec.workflows) {
            ...
            writeFile(path.join(base, "workflows", `${wSlug}-${w.id}.md`),
                page({ type: "Workflow", id: w.id, name: w.name, slug: wSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const s of spec.workflowSteps) {
            ...
            writeFile(path.join(base, "steps", `${String(s.order).padStart(2, "0")}-${sSlug}-${s.id}.md`),
                page({ type: "WorkflowStep", id: s.id, name: s.name, slug: sSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const g of spec.phaseGates) {
            ...
            writeFile(path.join(base, "gates", `${gSlug}-${g.id}.md`),
                page({ type: "PhaseGate", id: g.id, name: g.name, slug: gSlug }, rels, shorthand, children, cfg.storage));
        }
        ...
        for (const t of spec.gateTransitions) {
            ...
            writeFile(path.join(base, "transitions", `${tSlug}-${t.id}.md`),
                page({ type: "GateTransition", id: t.id, name: t.name, slug: tSlug }, rels, shorthand, [], cfg.storage));
        }
    }
}
