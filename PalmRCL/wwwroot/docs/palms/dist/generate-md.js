...
function loadConfig() {
    return readJSON(path.join(ROOT, "palms.config.json"));
}
function loadStates() {
    return readJSON(path.join(DATA_DIR, "workflow-states.json"));
}
...
function detUuid(ns, ...parts) { return uuidv5(parts.join("::"), ns); }
...
function parseFolder(folder) {
    ...
    const get = (rel) => path.join(base, rel);
    const readAll = (dirRel) => {
    ...
    const palm = matter(palmFile).data;
    ...
    return { palm, models, lifecycles, phases, workflows, steps, gates, transitions };
}
function applyDeterministicIds(cfg, g) {
    const ns = cfg.namespaceUUID;
    const fix = (t, x, parent) => {
        x.slug = x.slug ?? slugify(x.name);
        x.id = x.id ?? detUuid(ns, t, parent ?? "", x.name);
    };
    fix("PaLM", g.palm);
    for (const m of g.models)
        fix("Model", m, g.palm.id);
    for (const l of g.lifecycles)
        fix("Lifecycle", l, JSON.stringify(l.modelIds ?? l.modelNames ?? []));
    for (const p of g.phases)
        fix("LifecyclePhase", p, p.lifecycleId ?? p.lifecycleName ?? "");
    for (const w of g.workflows)
        fix("Workflow", w, w.phaseId ?? w.phaseName ?? "");
    for (const s of g.steps)
        fix("WorkflowStep", s, s.workflowId ?? s.workflowName ?? "");
    for (const gt of g.gates)
        fix("PhaseGate", gt);
    for (const t of g.transitions)
        fix("GateTransition", t, t.gateId ?? t.gateName ?? "");
}
function resolveNamesToIds(g) {
    // Build name->ID maps (case-insensitive)
    const mapBy = (arr) => {
        const m = new Map();
        for (const x of arr)
            m.set(x.name.toLowerCase(), x.id);
        return m;
    };
    const M = mapBy(g.models);
    const L = mapBy(g.lifecycles);
    const P = mapBy(g.phases);
    const W = mapBy(g.workflows);
    const S = mapBy(g.steps);
    const G = mapBy(g.gates);
    ...
    for (const l of g.lifecycles) {
        if ((!l.modelIds || !l.modelIds.length) && l.modelNames && l.modelNames.length) {
            l.modelIds = l.modelNames.map(n => {
                ...
                if (!id)
                    throw new Error(`Lifecycle '${l.name}': modelName '${n}' not found`);
                return id;
            });
        }
    }
    ...
    for (const p of g.phases) {
        if (!p.lifecycleId && p.lifecycleName) {
            ...
            if (!id)
                throw new Error(`Phase '${p.name}': lifecycleName '${p.lifecycleName}' not found`);
            p.lifecycleId = id;
        }
    }
    // Workflow phaseId
    for (const w of g.workflows) {
        if (!w.phaseId && w.phaseName) {
            ...
            if (!id)
                throw new Error(`Workflow '${w.name}': phaseName '${w.phaseName}' not found`);
            w.phaseId = id;
        }
    }
    // Step workflowId
    for (const s of g.steps) {
        if (!s.workflowId && s.workflowName) {
            ...
            if (!id)
                throw new Error(`Step '${s.name}': workflowName '${s.workflowName}' not found`);
            s.workflowId = id;
        }
    }
    ...
    for (const t of g.transitions) {
        if (!t.gateId && t.gateName) {
            ...
            if (!id)
                throw new Error(`Transition '${t.name}': gateName '${t.gateName}' not found`);
            t.gateId = id;
        }
        if (t.targetPhaseName) {
            const id = P.get(t.targetPhaseName.toLowerCase());
            if (!id)
                throw new Error(`Transition '${t.name}': targetPhaseName '${t.targetPhaseName}' not found`);
            t.targetPhaseId = id;
        }
        if (t.targetStepName) {
            const id = S.get(t.targetStepName.toLowerCase());
            if (!id)
                throw new Error(`Transition '${t.name}': targetStepName '${t.targetStepName}' not found`);
            t.targetStepId = id;
        }
    }
}
function validate(g, states) {
    const must = (cond, msg) => { if (!cond)
        throw new Error(`Spec invalid: ${msg}`); };
    const byId = (arr) => new Set(arr.map(x => x.id));
    const M = byId(g.models);
    const L = byId(g.lifecycles);
    const P = byId(g.phases);
    const W = byId(g.workflows);
    const S = byId(g.steps);
    const G = byId(g.gates);
    const stateIds = new Set(states.map(s => s.id));
    for (const m of g.models)
        for (const sid of (m.workflowStates ?? []))
            must(stateIds.has(sid), `Model '${m.name}' refs missing WorkflowState ${sid}`);
    for (const l of g.lifecycles) {
        must((l.modelIds ?? []).length > 0, `Lifecycle '${l.name}' must link ≥1 model`);
        for (const mid of (l.modelIds ?? []))
            must(M.has(mid), `Lifecycle '${l.name}' refs missing Model ${mid}`);
    }
    // Phase orders
    const phasesByLifecycle = new Map();
    for (const p of g.phases) {
        must(!!p.lifecycleId, `Phase '${p.name}' missing lifecycleId`);
        if (!phasesByLifecycle.has(p.lifecycleId))
            phasesByLifecycle.set(p.lifecycleId, []);
    ...
    for (const [lc, arr] of phasesByLifecycle.entries()) {
        ...
        for (const x of arr)
            must(x.order > 0, `Lifecycle ${lc} has non-positive PhaseOrder`);
    }
    // Workflow 1:1 Phase
    const wfByPhase = new Map();
    for (const w of g.workflows) {
        must(!!w.phaseId && P.has(w.phaseId), `Workflow '${w.name}' refs missing Phase`);
        wfByPhase.set(w.phaseId, (wfByPhase.get(w.phaseId) ?? 0) + 1);
    }
    for (const [pid, cnt] of wfByPhase.entries())
        must(cnt === 1, `Phase ${pid} must have exactly 1 workflow (found ${cnt})`);
    ...
    for (const st of g.steps) {
        ...
        if (!stepsByWf.has(st.workflowId))
            stepsByWf.set(st.workflowId, []);
    ...
    for (const gte of g.gates) {
        for (const ph of (gte.anchor?.phases ?? []))
            must(P.has(ph), `Gate '${gte.name}' anchor phase missing: ${ph}`);
        for (const st of (gte.anchor?.steps ?? []))
            must(S.has(st.step), `Gate '${gte.name}' anchor step missing`);
    }
    // Transitions per gate: ≥1 & exactly one default; absolute vs relative shape
    const transByGate = new Map();
    for (const t of g.transitions) {
        must(!!t.gateId && G.has(t.gateId), `Transition '${t.name}' refs missing Gate`);
        if (!transByGate.has(t.gateId))
            transByGate.set(t.gateId, []);
    ...
    for (const [gid, arr] of transByGate.entries()) {
        ...
        for (const t of arr) {
            if (t.mode === "Relative") {
                ...
                if (t.targetPhaseId)
                    must(P.has(t.targetPhaseId), `Transition '${t.name}' targets missing Phase`);
                if (t.targetStepId)
                    must(S.has(t.targetStepId), `Transition '${t.name}' targets missing Step`);
            }
        }
    }
}
function writeDocs(cfg, states, folderName, g) {
...
`;
    ...
    // Helpers
    const page = (meta, relations, shorthand, children) => `${fm({
...
`;
    ...
    for (const m of g.models) {
        ...
        writeFile(path.join(base, "models", `${mSlug}-${m.id}.md`), page(m, rels, shorthand, children));
    }
    // Lifecycles
    for (const l of g.lifecycles) {
        ...
        writeFile(path.join(base, "lifecycles", `${lSlug}-${l.id}.md`), page(l, rels, shorthand, children));
    }
    // Phases
    for (const ph of g.phases) {
        const phSlug = ph.slug ?? slugify(ph.name);
        const wf = g.workflows.find(w => w.phaseId === ph.id);
        const phaseGateIds = new Set(g.gates.flatMap(gt => (gt.anchor?.phases ?? [])).filter(x => x === ph.id));
        const gatesAtPhase = g.gates.filter(gt => (gt.anchor?.phases ?? []).includes(ph.id));
        const rels = [
        ...
        writeFile(path.join(base, "phases", `${String(ph.order).padStart(2, "0")}-${phSlug}-${ph.id}.md`), page(ph, rels, shorthand, children));
    }
    // Workflows
    for (const w of g.workflows) {
        ...
        writeFile(path.join(base, "workflows", `${wSlug}-${w.id}.md`), page(w, rels, shorthand, children));
    }
    // Steps
    for (const s of g.steps) {
        const sSlug = s.slug ?? slugify(s.name);
        const stepGates = g.gates.filter(gt => (gt.anchor?.steps ?? []).some(x => x.step === s.id));
        const rels = [
        ...
        writeFile(path.join(base, "steps", `${String(s.order).padStart(2, "0")}-${sSlug}-${s.id}.md`), page(s, rels, shorthand, children));
    }
    // Gates
    for (const gte of g.gates) {
        ...
        writeFile(path.join(base, "gates", `${gSlug}-${gte.id}.md`), page(gte, rels, shorthand, children));
    }
    // Transitions
    for (const t of g.transitions) {
        ...
        writeFile(path.join(base, "transitions", `${tSlug}-${t.id}.md`), page(t, rels, shorthand, []));
    }
}
async function run() {
    ...
    if (!folders.length)
        throw new Error(`No folders found under ${SPECS_DIR}`);
